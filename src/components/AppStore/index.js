...
class AppStore extends Component {
  state = {appCategory: appsList[0].category, searchinput: ''}

  appsListFiltered = () => {
    const {appCategory, searchinput} = this.state
    const filterdList = appsList.filter(
      each =>
        appCategory === each.category &&
        each.appName.toLowerCase().includes(searchinput.toLowerCase()),
    )
    return filterdList
  }

  onClickTabDetails = tabId => {
  ...
  render() {
    ...
    const filterdList = this.appsListFiltered()
    return (
        ...
        <ul className="tab-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              onClickTabItem={this.onClickTabDetails}
